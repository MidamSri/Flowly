import { Action, SuccessfulPattern, FailurePattern } from '@flowly/shared';

/**
 * Computes a unique string signature for a successful pattern based on goal + actions.
 */
export function computeSuccessfulPatternHash(goal: string, actions: Action[]): string {
 ...
 */
export function computeFailurePatternHash(goal: string, failedStep: number, error: string): string {
  const normalizedGoal = goal.trim().toLowerCase();
 ...
 */
export function deduplicateAndAddSuccess(
  existing: SuccessfulPattern[],
  newPattern: SuccessfulPattern
): SuccessfulPattern[] {
 ...
 */
export function deduplicateAndAddFailure(
